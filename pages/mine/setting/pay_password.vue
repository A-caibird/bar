<template>
	<view class="container">
		<view class="header-box">
			<u-navbar back-icon-color="#ffffff" title="设置支付密码" title-size="36" title-color="#FFFFFF" :background="{ background: '#16192B' }"
...
<script>
	...
	export default {
		...
		computed: {
			btnActive: function() {
				if (this.newPayPassword.length==6 && this.repeatPassword.length==6) {
					return true;
				} else {
					return false;
				}
		...
		methods: {
			setHandle: function() {
				...
				this.setNewPayPassword({
					payPassword: params.newPayPassword,
				});
			},
			//设置新支付密码
			setNewPayPassword: function(params) {
				console.log(params)
				this.$u.api.setPayPasswordApi(params).then(res => {
					var code = res.code;
					if (parseInt(code) == 0) {
						this.$u.toast('设置支付密码成功');
						uni.$emit('setPayPassword')
						setTimeout(() => {
							this.$u.route({
								type:'back',
							})
