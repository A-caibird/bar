<template>
	<view class="container">
		<scroll-view scroll-y="true" :scroll-into-view="scrollBottom" class="chat-content" id="scrollview" @tap="closeAllPop"
		 @scrolltoupper="loadMoreChatList" :style="{height:chatType=='greet'||chatType=='reply'?'calc(100vh - 340rpx)':'calc(100vh - 100rpx)'}">
			...
			<view class="chat-item" v-for="(item,index) in chatList" :key="index" :id="`chat${index}`">
				...
				<view class="msg-warp" :class="[item.fromId==friendUserInfo.chatToken ? 'left' : 'right']">
					<view class="avatar" @tap="$u.throttle(goPersonalHomepage(item.fromId==friendUserInfo.chatToken?friendUserInfo.userId:userInfo.userId,item.fromId!=friendUserInfo.chatToken))">
						<image :src="item.fromId==friendUserInfo.chatToken?friendUserInfo.localAvatar:(userInfo.hasSave?userInfo.localAvatar:userInfo.avatar)" />
					</view>
					<view style="flex:1;display: flex;flex-direction: column;">
						...
						<view class="msg-content">
							...
							<view class="image" v-else-if="item.type==1">
								<image @tap="$u.throttle(previewImg(`file://${item.src}`))" mode="widthFix" :src="`file://${item.src}`"></image>
							</view>
	...
	export default {
		...
		methods: {
			tapSendReply(msg){
				...
				$chat.sendMsg(userInfo,friendUserInfo,'single','reply',{content: msg,})
			},
			tapSendGreet(msg){
				let userInfo = this.$u.deepClone(this.userInfo)
				let friendUserInfo = this.$u.deepClone(this.friendUserInfo)
				$chat.sendMsg(userInfo,friendUserInfo,'single','greet',{content: msg,})
			},
			async tapAgreeJoinPing(item){
				...
				if(code==0) {
					...
					$chat.upadteChatByTimeType(userInfo.chatToken, friendUserInfo.chatToken,item.time,item.type,item)
					$chat.sendMsg(userInfo,friendUserInfo,'single','text',{content: '同意加入',})
			...
			async tapRefuseJoinPing(item){
				...
				$chat.sendMsg(userInfo,friendUserInfo,'single','text',{content: '拒绝加入',})
			},
			...
			async refuseYaoyue(e){
				...
				console.log(e)
				let {item,reason} = e
				let {code} = await this.$u.api.refuseYaoyueApi({
				...
				if(code==0) {
					...
					$chat.upadteChatByTimeType(this.userInfo.chatToken, this.friendUserInfo.chatToken,item.time,item.type,item)
					// $chat.sendMsg(userInfo,friendUserInfo,'single','text',{content: '拒绝邀请',})
                    $chat.sendMsg(userInfo,friendUserInfo,'single','text',{content: e.reason,})
				}
			},
			...
			async getCanChat(){
				...
				let {code,data} = await this.$u.api.getCanChatApi({
			...
			async getOtherUserInfo(){
				let friendUserInfo = this.$u.deepClone(this.friendUserInfo)
				let {code,data} = await this.$u.api.otherMsgApi({
					userId:friendUserInfo.userId
				})
				if(code==0) {
					// console.log(data)
					...
					//下载头像 以及同步昵称、头像
					this.friendUserInfo.localAvatar = await $chat.setChatUserListNameAvatarFromStorageByUserId(this.userInfo.chatToken,friendUserInfo.userId,data.info.nickName,data.info.avatar)
					this.$forceUpdate()
				}
			},
			...
			async loadMoreChatList() {
				...
				console.log('下拉加载！')
				this.loadPage = true
				let list = await $chat.getChatList(this.userInfo.chatToken,this.friendUserInfo.chatToken,this.pageNumber)
			...
			msgPush(e) {
				...
				let chatToken = this.userInfo.chatToken
				let friendChatToken = this.friendUserInfo.chatToken
				if ((chatToken == e.toId && e.fromId == friendChatToken) || (e.toId == friendChatToken && e.fromId == chatToken)) {
			...
			sendMessage() {
				let userInfo = this.$u.deepClone(this.userInfo)
				let friendUserInfo = this.$u.deepClone(this.friendUserInfo)
				$chat.sendMsg(userInfo,friendUserInfo,'single','text',{content: this.message,})
				// 将消息推入聊天列表
			...
			sendFile(fileType, tempPath) {
				let userInfo = this.$u.deepClone(this.userInfo)
				let friendUserInfo = this.$u.deepClone(this.friendUserInfo)
				$chat.sendMsg(userInfo,friendUserInfo,'single',fileType,{tempPath: tempPath,})
