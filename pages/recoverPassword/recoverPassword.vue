<template>
	<view class="container">
		<view class="header_box">
			<u-navbar back-icon-color="#FFFFFF" title="找回密码" title-color="#FFFFFF" :border-bottom="false" :background="{'backgroundColor': '#16192B'}"></u-navbar>
		</view>
		<view class="middle_box">
			<view class="form_box">
				...
				<view class="form_item_panel">
					<view class="input_box">
						<input v-model="surePassword" placeholder-class="placeholder" placeholder="请再次输入新密码" />
					</view>
				</view>
			</view>
			<view class="register_btn" :class="{'active':buttonActive}" @tap="$u.throttle(setHandle, 600)"> <text>确认</text> </view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			setHandle: function() {
				...
				if (params.newPassword != this.surePassword && params.newPassword != '') {
					this.$u.toast('两次密码不一致，请修改');
					return
				}
			
				this.$u.api.forgetPasswordApi(params).then(res => {
					let code = res.code;
