	...
	<view>
		<mescroll-uni :ref="'mescrollRef'+i" :fixed="true" @init="mescrollInit" :down="downOption" @down="downCallback" :up="upOption" @up="upCallback">
			<block v-if="hasLocation">
				<view class="search_box" v-if="mode == 'nearby'">
				...
				<view  v-for="(info, index) in pageList" :key="index">
					...
					<block v-else>
						<find-item :mode="mode" :info="info" :showPercent="showPercent" @shareTap="$emit('shareTap')" @showYaoyue="handleShowYaoyue" @showPing="handleShowPing" @oepnComment="$emit('oepnComment',$event)" @oepnGift="$emit('oepnGift',$event)"></find-item>
					</block>
	...
	export default{
		...
		data(){
			return {
				...
				url:this.mode=='follow'?'/api/dynamic/myAttentionUserDynamic':'/api/dynamic/nearby',
				params:{
