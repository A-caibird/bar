		...
		<view class="header_box">
			...
			>
				<view style="width: calc(100% - 60rpx);margin-left: 30rpx; display: flex; align-items: center;">
					...
					<view class="search_box" style="width: calc(100% - 160rpx);" @tap="$u.throttle(tapGoClubList('search'))">
						<u-search
							:input-style="{ 'font-size': '28rpx', color: '#FFFFFF' }"
							placeholder-color="#9497B5"
							:clearabled="false"
							color="#9497B5"
							bgColor="#33365B"
							shape="round"
					...
					<view class="club_list">
						<block v-if="hasLocation">
							<block v-for="(info, index) in pageList" :key="index">
								<club :info="info"></club>
								<block v-if="index != pageList.length - 1"><u-gap :height="20" bgColor="#20234B"></u-gap></block>
							</block>
						</block>
...
export default {
	...
	methods: {
		...
		async getHomeBannerList() {
			let {code,data} = await this.$u.api.getHomeBannerListApi()
			if(code==0) {
