<template>
	<view class="mescroll-uni-warp">
		<scroll-view :id="viewId" refresher-background="transparent" class="mescroll-uni" :class="{'mescroll-uni-fixed':isFixed}" :style="{'height':scrollHeight,'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}" :scroll-top="scrollTop" :scroll-with-animation="scrollAnim" :upper-threshold="upperThreshold" @scroll="scroll" @scrolltoupper="$emit('scrolltoupper',$event)" :scroll-y='scrollable' :enable-back-to-top="true" :throttle="false">
		...
		computed: {
			...
			scrollHeight(){
				if (this.isFixed) {
					...
					return (this.height.indexOf('calc')==-1?this.height:this.toPx(this.height) + 'px')
				}else{
					return "100%"
				}
			},
			...
			// 列表是否可滑动
			scrollable(){
				return (this.downLoadType===0 || this.isDownReset)&& this.canScroll
			},
			// 是否在加载中
			isDownLoading(){
